...
class Learning:
	...
	def _evalPolicy(self,states):
		''' Policy evaluation step.'''
		delta = 0.0
		if len(states)==0:
			states=xrange(self.numStates)
		for s in states:
			v = self.V[s]
	...
	def _improvePolicy(self,states):
	...
	def solvePolicyIteration_subsetStates(self, states,theta=0.001):
		''' Implementation of Policy Iteration, as in the policy iteration
		    pseudo-code presented in Sutton and Barto (2016).'''
		...
		while not policy_stable:
			# Policy evaluation
			delta = self._evalPolicy(states)
			while (theta < delta):
				delta = self._evalPolicy(states)

			# Policy improvement
			policy_stable = self._improvePolicy(states)
